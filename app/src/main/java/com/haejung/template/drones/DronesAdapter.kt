...
class DronesAdapter(private val listener: DronesFragment.DroneItemListener) :
    ...
    var items: List<DroneSummary> by Delegates.observable(emptyList()) { _, _, _ -> notifyDataSetChanged() }
    ...
    class ViewHolder(private val view: View, private val listener: DronesFragment.DroneItemListener) :
        ...
        private var droneItem: DroneSummary? = null

        @SuppressLint("SetTextI18n")
        fun bind(drone: DroneSummary) {
            droneItem = drone
            droneItem?.apply {
